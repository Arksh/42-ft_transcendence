    ...
    if (phase === TurnManager.PHASES.FORTIFY) {
      ...
      if (!fortifyFrom) {
        if (territoryOwners[clickedId] !== currentPlayer.faction) {
          console.log(
            'No puedes fortificar desde un territorio que no posees:',
            clickedTerritory.name,
            '\nPropietario del territorio:',
            territoryOwners[clickedId],
            '\nFacción del jugador actual:',
            currentPlayer.faction
          );
          return;
        }
        if (troopCounts[clickedId] <= 1) {
          console.log(
            'No puedes fortificar desde un territorio con 1 tropa o menos:',
            clickedTerritory.name,
            '\nPropietario del territorio:',
            territoryOwners[clickedId],
            '\nFacción del jugador actual:',
        ...
        console.log(
          'Seleccionado para fortificar desde:',
          clickedTerritory.name,
          '\nPropietario del territorio:',
          territoryOwners[clickedId],
          '\nFacción del jugador actual:',
      ...
      // 2do clic en fase FORTIFY: selecciona el territorio de destino y mueve tropas
      if (territoryOwners[clickedId] !== currentPlayer.faction) {
        console.log(
          'No puedes fortificar un territorio que no posees:',
          clickedTerritory.name,
          '\nPropietario del territorio:',
          territoryOwners[clickedId],
          '\nFacción del jugador actual:',
      ...
      if (!TERRITORIES[fortifyFrom].neighbors.includes(clickedId)) {
        console.log(
          'No puedes fortificar un territorio que no es vecino:',
          clickedTerritory.name,
          '\nPropietario del territorio:',
          territoryOwners[clickedId],
          '\nFacción del jugador actual:',
      ...
      if (clickedId === fortifyFrom) {
        setFortifyFrom(null);
        console.log(
          'Deseleccionado fortificar desde:',
          clickedTerritory.name,
          '\nPropietario del territorio:',
          territoryOwners[clickedId],
          '\nFacción del jugador actual:',
      ...
      setTroopCounts((prev) => ({
        ...prev,
        [fortifyFrom]: prev[fortifyFrom] - 1,
        [clickedId]: prev[clickedId] + 1,
      }));
      setFortifyFrom(null);
      console.log(`Movido 1 tropa de ${fortifyFrom} a ${clickedId}`);
    }
  }
      ...
      {selectedTerritory && (
        <p>
          ...
          {territoryOwners[selectedTerritory.id]
            ...
            : 'Neutral'}
          <br></br>
          {selectedTerritory.name} — Capital: {selectedTerritory.capital}
          <br></br>
          Tropas: {troopCounts[selectedTerritory.id]}
        </p>
      )}
      <canvas
        ref={canvasRef}
